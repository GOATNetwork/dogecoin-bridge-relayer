...
func (w *WalletClient) ProcessConsolidation(task *db.Consolidation) {
	...
	if balance.Cmp(task.Amount) == 1 {
		w.processConsolidationTxSign(task)
	}
}

func (w *WalletClient) processConsolidationTxSign(task *db.Consolidation) {
	hotAddress := address.HotAddressOfSolana(w.tss.ECPoint(w.ChainType()))
	log.Infof("hotAddress: %v,targetAddress: %v, amount: %v", hotAddress, task.FromAddress, task.Amount)
	...
	if task.IsNative() {
		tx, err = w.client.BuildSolTransferWithAddress(task.FromAddress, hotAddress, task.Amount.BigInt().Uint64())
	} else {
		tx, err = w.client.BuildTokenTransfer(task.ContractAddress, task.FromAddress, hotAddress, task.Amount.BigInt().Uint64(), task.Decimals)
	}
