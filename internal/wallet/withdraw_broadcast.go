...
func NewOrderBroadcaster(btcClient *rpcclient.Client, state *state.State) OrderBroadcaster {
	...
	// Choose to use GigaWallet or other client based on configuration
	if config.AppConfig.UseGigaWallet {
		orderBroadcaster.remoteClient = NewGigaWalletClient(config.AppConfig.GigaWalletURL, config.AppConfig.GigaWalletAccountID)
	} else if config.AppConfig.BTCNetworkType == "regtest" {
