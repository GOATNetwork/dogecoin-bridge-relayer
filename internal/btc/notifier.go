...
func NewBTCNotifier(client *rpcclient.Client, cache *BTCCache, poller *BTCPoller, chainInfo config.Chain) *BTCNotifier {
	...
	if maxBlockHeight < int64(chainInfo.StartHeight) {
		maxBlockHeight = int64(chainInfo.StartHeight) - 1
	}
	...
	if resultQuery.Error != nil && errors.Is(resultQuery.Error, gorm.ErrRecordNotFound) {
		syncStatus.ConfirmedHeight = int64(chainInfo.StartHeight - 1)
		syncStatus.UnconfirmHeight = int64(chainInfo.StartHeight - 1)
